...
void initialize(DynetParams& params) {
  ...
  // hltc fork: random device is now a thread_local object and are initiaized statically
  // Set random seed
...
void reset_rng(unsigned seed) {
  rndeng = std::make_shared<mt19937>(seed);
#if HAVE_CUDA
  DeviceManager* device_manager = get_device_manager();
...
void preallocate_thread_local_mempool() {
  if(!default_device) throw std::runtime_error("Please initialize first");
  default_device->ensure_init_thread_local_pools();
}
