...
Expression concatenate(const std::vector<Expression> &xs, unsigned d) { return detail::f<Concatenate>(xs, d); }
  ...
  Expression to_boolean(const Expression& x) { return Expression(x.pg, x.pg->add_function<ToBoolean>({x.i})); }

  Expression soft_ifelse(const Expression& cond, const Expression& val_if_true, const Expression& val_if_false) {
    return Expression(cond.pg, cond.pg->add_function<ConditionalExpression>({cond.i, val_if_true.i, val_if_false.i}));
  }

  Expression operator!(const Expression& x) {
    return Expression(x.pg, x.pg->add_function<LogicalNot>({x.i}));
  }

  Expression operator&&(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<LogicalAnd>({x.i, y.i}));
  }

  Expression operator||(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<LogicalOr>({x.i, y.i}));
  }

  Expression cwiseEq(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<CwiseEq>({x.i, y.i}));
  }

  Expression cwiseNe(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<CwiseNe>({x.i, y.i}));
  }

  Expression operator<(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<CwiseLt>({x.i, y.i}));
  }

  Expression operator>(const Expression& x, const Expression& y) {
    return Expression(x.pg, x.pg->add_function<CwiseGt>({x.i, y.i}));
  }
