...
namespace dynet {
  ...
  thread_local std::unordered_map<const Device*, std::vector<std::shared_ptr<AlignedMemoryPool>>> Device::thread_local_pools;
...
DeviceMempoolSizes Device::mark(ComputationGraph *cg) {
  ...
  return DeviceMempoolSizes(get_pool(0)->used(), get_pool(1)->used(), get_pool(2)->used(), get_pool(3)->used());
}

void Device::revert(const DeviceMempoolSizes & cp) {
  if(cp.used[0] > get_pool(0)->used())
    DYNET_INVALID_ARG("Saved value greater than original value in Device::revert (" << cp.used[0] << " > " << get_pool(0)->used() << ")");
  get_pool(0)->set_used(cp.used[0]);
  if(cp.used[1] > get_pool(1)->used())
    DYNET_INVALID_ARG("Saved value greater than original value in Device::revert (" << cp.used[1] << " > " << get_pool(1)->used() << ")");
  get_pool(1)->set_used(cp.used[1]);
  if(cp.used[2] > get_pool(2)->used())
    DYNET_INVALID_ARG("Saved value greater than original value in Device::revert (" << cp.used[2] << " > " << get_pool(2)->used() << ")");
  get_pool(2)->set_used(cp.used[2]);
  if(cp.used[3] > get_pool(3)->used())
    DYNET_INVALID_ARG("Saved value greater than original value in Device::revert (" << cp.used[3] << " > " << get_pool(3)->used() << ")");
  get_pool(3)->set_used(cp.used[3]);
}
...
void Device::allocate_tensor(DeviceMempool mp, Tensor & tens) {
  ...
  tens.v = (float*)get_pool((int)mp)->allocate(tens.d.size() * sizeof(float));
  DYNET_ASSERT(tens.v != nullptr, "Allocated tensor is zero");
  tens.mem_pool = mp;
}

std::shared_ptr<AlignedMemoryPool> Device::get_pool(unsigned int pool) {
  ensure_init_thread_local_pools();
  if(pool == 2) {
    return shared_pool;
  }
  return thread_local_pools[this].at(pool);
}

void Device::ensure_init_thread_local_pools() {
  auto& my_pools = thread_local_pools[this];
  if(my_pools.empty()) {

    my_pools = {
      create_pool(sizes_mb.used[0] << 20),
      create_pool(sizes_mb.used[1] << 20),
      nullptr,
      create_pool(sizes_mb.used[3] << 20)
    };
...
Device_GPU::Device_GPU(int my_id, const DeviceMempoolSizes & mbs,
  ...
  Device(my_id, DeviceType::GPU, &gpu_mem, mbs), cuda_device_id(device_id), gpu_mem(device_id) {
  ...
  shared_pool = create_pool(mbs.used[2] << 20);
//  get_pool(0) = new AlignedMemoryPool("GPU forward memory", (mbs.used[0] << 20), &gpu_mem);
//  get_pool(1) = new AlignedMemoryPool("GPU backward memory", (mbs.used[1] << 20), &gpu_mem);
//  get_pool(2) = new AlignedMemoryPool("GPU parameter memory", (mbs.used[2] << 20), &gpu_mem);
//  get_pool(3) = new AlignedMemoryPool("GPU scratch memory", (mbs.used[3] << 20), &gpu_mem);
}

Device_GPU::~Device_GPU() {}

std::shared_ptr<AlignedMemoryPool> Device_GPU::create_pool(size_t initial_cap) {
    return std::make_shared<AlignedMemoryPool>("GPU memory",initial_cap, &gpu_mem);
  }
...
Device_CPU::Device_CPU(int my_id, const DeviceMempoolSizes & mbs, bool shared) :
  Device(my_id, DeviceType::CPU, &cpu_mem, mbs), shmem(mem) {
  ...
  *kSCALAR_ZERO = 0;
  ...
  // this is the big memory allocation.
  shared_pool = create_pool(mbs.used[2] << 20);
//  get_pool(0) = new AlignedMemoryPool("CPU forward memory", (mbs.used[0] << 20), &cpu_mem);
//  get_pool(1) = new AlignedMemoryPool("CPU backward memory", (mbs.used[1] << 20), &cpu_mem);
//  get_pool(2) = new AlignedMemoryPool("CPU parameter memory", (mbs.used[2] << 20), shmem);
//  get_pool(3) = new AlignedMemoryPool("CPU scratch memory", (mbs.used[3] << 20), &cpu_mem);
}
