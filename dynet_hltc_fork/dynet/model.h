...
//  std::unordered_set<unsigned> non_zero_grads; /**< Gradients are sparse, so track which components are nonzero */
  ...
  std::list<unsigned> non_zero_grads() const {
    std::list<unsigned> ret;
    for(unsigned i=0; i<is_grads_non_zero.size(); ++i) {
      if(is_grads_non_zero[i]) ret.push_back(i);
    }
    return ret;
  };
  std::vector<bool> is_grads_non_zero; /**< Gradients are sparse, so track which components are nonzero */
